...
how_to = box(
   title = 'どうやって使用するの？(ユースケース２を推奨しています)',
   h4(br(),'ユースケース１：web上で、サンプルデータなどを用いてお楽しみください'),
   h5('',br()),
   h4('ユースケース２：ご自身のPCなどで、本サービスをご利用ください(推奨)'),
   h5('皆様自身のPCなどでご利用いただけるように、以下のリンクから無料でソースコードを提供しております。'),
   uiOutput('github_page_box_how_to'),
   uiOutput('how_to_download_box_how_to'),
   uiOutput('how_to_use_box_how_to'),
...
security = box(
   title = 'セキュリティについて',
   h5('本ウェブサイトはSSL通信を実施しています',br(),'ただし、どなたでもアクセスできるようになっております。',br(),'そのため、当webアプリでは下記の2つのユースケースを想定しております。'),
   h4(br(),'ユースケース１：web上で、サンプルデータなどを用いてお楽しみください'),
   h5('機密性の高いデータを用いた利用をお薦めしておりません。',br(),'サンプルデータで本サービスをお楽しみ下さることを強く推奨いたします。',br(),'もし本サービスご利用後、今後も使ってみたいなとなりましたら、ユースケース２をご覧ください。'),
   h5('',br()),
   h4('ユースケース２：ご自身のPCなどで、本サービスをご利用ください(推奨)'),
   h5('皆様自身のPCなどでご利用いただけるように、以下のリンクから無料でソースコードを提供しております。'),
   uiOutput('github_page_box_security'),
   uiOutput('how_to_download_box_security'),
   uiOutput('how_to_use_box_security'),
   collapsible = TRUE, collapsed = TRUE, width = home_box_width
)
...
update = h5( 'version 3.0', br(), 'updated: 8th October, 2020', br(), '© 2020 Yusuke Ito' );
